...
async function controllerHarness() {
 ...
 sandbox.window=sandbox;sandbox.ImpactCore=core;sandbox.RecordFiles={...files,openStore:async()=>storage,buildExport:(s,a,l)=>files.buildExport(s,a,l,core)};sandbox.NoiseInput={ensure:async()=>({context,stream:{getAudioTracks:()=>[track]},source:{connect(){},disconnect(){}}}),setRecording(){},frequencyX:()=>50,latestHistory:()=>null};sandbox.addEventListener=()=>{};
 vm.runInNewContext(fs.readFileSync(path.join(root,'record-logging.js'),'utf8'),sandbox);
 await new Promise(resolve=>setImmediate(resolve));await get('recordStart').onclick();assert.equal(recorder.state,'recording');assert.equal(sandbox.ImpactLogging.isRecording(),true);assert.equal(get('detect-triggerDb').disabled,true);
 ...
 failChunks=false;await get('recordExport').onclick();assert.equal(get('recordConfirm').disabled,false);assert.equal(get('recordDelete').disabled,true);await get('recordConfirm').onclick();assert.equal(get('recordDelete').disabled,false);
