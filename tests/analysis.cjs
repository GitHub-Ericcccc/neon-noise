...
(async()=>{
 ...
 assert.equal(run('window.NoiseInput.historyWindowMs'),60000);
 run('captureStartedAt=0; capturedElapsedMs=0; historyFrames=[]; pushWaterfallRow(0);pushWaterfallRow(30000);pushWaterfallRow(60000)');
 assert.equal(run('historyFrames.length'),3);
 run('pushWaterfallRow(60001)');assert.equal(run('historyFrames[0].timestamp'),30000);
 run('captureStartedAt=0; capturedElapsedMs=0; historyFrames=[]; pushWaterfallRow(0);pushWaterfallRow(10000);pushWaterfallRow(61000)');
 ...
 run('historyFrames=[];for(let n=0;n<800;n++)pushWaterfallRow(n*110)');assert.ok(run('historyFrames.length')<=546);
 ...
 console.log('PASS: syntax, coordinate roundtrip, dB mapping, peak decay/hold, four analysis modes, peak frequency, 60-second history pruning, raster cache invalidation, clear, microphone timeout/late stream release, independent color range, embedded MIT.');
