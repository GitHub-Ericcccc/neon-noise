...
(function() {
  ...
  function interrupt(reason) {
    ...
    const t=elapsed();interrupted=true;node?.port.postMessage({type:'enabled',enabled:false});sealSegment(t);eventPush(detector.interrupt(t));session.interruptions.push({type:'interruption',timeMs:t,reason});session.elapsedMs=t;void save();render();say('已中断：'+reason);
  }
  ...
  $('analysisMode').addEventListener('change',()=>{drawMarkers();});
